...
void dfs(int cnt, int capacity, int usr)
{
	if (cnt == M)
	{
		if (capacity <= C)
	...
	for (int i = 0; i < M; i++)
	{
		...
		selected[i] = 1;
		if (usr == 0)
			dfs(cnt + 1, capacity + board[a[i].first][a[i].second], usr);
		else
			dfs(cnt + 1, capacity + board[b[i].first][b[i].second], usr);
		selected[i] = 0;
	}
	dfs(cnt + 1, capacity, usr);
}
